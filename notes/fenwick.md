...
我們假設 `arr[3]` 的值從 `0` 變成了 `5`，這個時候所有包含 `arr[3]` 的 partial sum 都要加 5。
...
假設我們要 update `arr[p]`，很明顯，第一個要 update 的 sum 當然就是 `sum[p]`，問題是，下一個呢？

當 `sum[t]` 要包含 `p` ，就需要這個條件：

```
left(t) < p <= t
```

有趣的是，只要我們決定了 `t` 的最右邊的 `1` 在甚麼位置，`t` 其實最多只有一個選擇。

假如我們選擇這個是 `t` 的最後一個 `1` bit 是第 2 bit。
...
換句話說，由 `p` 變 `t`，只不過係換咗條尾，由原本一個 `0xx` 變成 `100`，而且 `xx` 唔可以係全部係 `0`。
...
## 超方便公式
由 `t = p` 開始，呢條式`x + (x & -x)` 一次過滿足晒上面兩個願望，正好是下一個 `t`。
